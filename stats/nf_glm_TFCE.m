...
% 2) dataY - a matrix of data to do regression with
% 3) taxis - time points considered for analysis (must match time points
%   contained in dataX)
% 4) faxis - frequency points considered (must match frequency points in
%   dataX)
% 5) chanlocs - channel locations for scalp data. Must be in EEGLAB format
...
if ~isempty(chanlocs)
    if ~par
        tfc = ept_TFCE_glm( dataX, dataY , chanlocs, 'nperm', nperm);
    else
        tfc = ept_TFCE_glm_par( dataX, dataY , chanlocs, 'nperm', nperm);
    end
else
    if ~par
        tfc = ept_TFCE_glm( dataX, dataY , [], 'nperm', nperm, 'flag_ft', 1);
    else
        tfc = ept_TFCE_glm_par( dataX, dataY , [], 'nperm', nperm, 'flag_ft', 1);
    end
end
...
tfce.data = dataX;
tfce.corrvar = dataY;
tfce.obs = squeeze(mean(dataX));
tfce.sd = squeeze(std(dataX));
tfce.tstat = tfc.Obs;
...
disp(['Recommended methods text: correlations with significance testing' ... 
    ' with correction for ' ...
